...
- **Demo 1(收訊端視角)** — 看訊息如何送達王里長、值班人員、不會用 通訊軟體的李老伯
...
| Webhook | 即時訂閱 | 其他訂閱系統 |
| telegram / Email / SMS | 終端推播 | 里長、值勤、長者 |
| 公開狀態頁 | 透明稽核 | 一般民眾、媒體 |
...
本次 MVP 以 Telegram 實作推播與抵達確認，LINE 為 Phase 2 規劃管道
---
...
- [x] 2026/05/15 — 設計文件完成、初步送件
- [x] 2026/05/16–5/31 衝刺週(整合 + AI Agent(設計) + UI + Demo 影片)
- [x] 2026/06/01 — MVP 完成、最終送件(架構)

## ✅ MVP 實測狀態（2026/07）
- ✅ 水利署 IoW 即時水位 OpenData 介接（真實資料）
- ✅ 狀態變化偵測 + 事件寫入 Google Sheets
- ✅ Telegram Bot 分眾推播四種角色（里長/主管/現場/志工）
- ✅ 抵達確認層：inline 按鈕回寫 ackStatus + 時間戳（送達≠抵達）
- 🔧 Gemini AI 三 Agent：設計測試，Phase 2 實作
- 🔧 EMIC/CAP 匯出：介面預留，Phase 2 實作
---
